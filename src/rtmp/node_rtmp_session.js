...
const AV = require("./decode-encode/node_core_av");
const { AUDIO_SOUND_RATE, AUDIO_CODEC_NAME, VIDEO_CODEC_NAME } = require("./decode-encode/node_core_av");

const AMF = require("./decode-encode/node_core_amf");
...
class NodeRtmpSession {
  ...
  generateNewSessionID() {
    ...
    } while (this.server.sessions.has(sessionID))
    return sessionID;
  }
  run() {
  ...
  stop() {
    if (this.isStarting) {
      ...
      this.server.emit("doneConnect", this.id, this.connectCmdObj);
  ...
  rtmpAudioHandler() {
    ...
    for (let playerId of this.players) {
      let playerSession = this.server.sessions.get(playerId);

      if (playerSession.numPlayCache === 0) {
        playerSession.res.cork();
      }

      //@todo : inutile car que du rtmp
  ...
  onConnect(invokeMessage) {
    ...
    this.server.emit("preConnect", this.id, invokeMessage.cmdObj);
    if (!this.isStarting) {
      return;
    }
    ...
    this.server.emit("postConnect", this.id, invokeMessage.cmdObj);
  }
  ...
  async onPublish(invokeMessage) {
    ...
    this.publishStreamName = invokeMessage.streamName.split('?')[0];
    this.publishStreamPath = "/" + this.appname + "/" + this.publishStreamName;
    ...
    this.server.emit("prePublish", this.id, this.publishStreamPath, this.publishArgs, this.publishStreamName);
    if (!this.isStarting) {
      return;
    }
    ...
    try{
      ...
      Logger.log(`[rtmp publish] Authorized. id=${this.id} streamPath=${this.publishStreamPath} streamId=${this.publishStreamId} sign=${this.publishArgs.sign} `);
    } catch (e) {
      Logger.log(`[rtmp publish] Unauthorized. id=${this.id} streamPath=${this.publishStreamPath} streamId=${this.publishStreamId} sign=${this.publishArgs.sign} `,e.message);
      this.sendStatusMessage(this.publishStreamId, "error", "NetStream.publish.Unauthorized", "Authorization required.");
      this.stop();
      return;
    }
    this.publishStreamPath = this.publishMetaData?.publishStreamPath || this.publishStreamPath;


    if (this.server.publishers.has(this.publishStreamPath)) {
      this.reject();
      ...
      this.server.publishers.set(this.publishStreamPath, this.id);
      this.isPublishing = true;

      this.sendStatusMessage(this.publishStreamId, "status", "NetStream.Publish.Start", `${this.publishStreamPath} is now published.`);
      for (let idlePlayerId of this.server.idlePlayers) {
        let idlePlayer = this.server.sessions.get(idlePlayerId);
        if (idlePlayer && idlePlayer.playStreamPath === this.publishStreamPath) {
          idlePlayer.onStartPlay();
          this.server.idlePlayers.delete(idlePlayerId);
        }
      }
      this.server.emit("postPublish", this.id, this.publishStreamPath, this.publishArgs, this.publishStreamName, this.publishMetaData);
    }
  }
  ...
  onPlay(invokeMessage) {
    ...
    this.server.emit("prePlay", this.id, this.playStreamPath, this.playArgs);
    ...
    if (this.server.publishers.has(this.playStreamPath)) {
      this.onStartPlay();
    } else {
      Logger.log(`[rtmp play] Stream not found. id=${this.id} streamPath=${this.playStreamPath}  streamId=${this.playStreamId}`);
      this.isIdling = true;
      this.server.idlePlayers.add(this.id);
    }
  }

  onStartPlay() {
    let publisherId = this.server.publishers.get(this.playStreamPath);
    let publisher = this.server.sessions.get(publisherId);
    ...
    this.isIdling = false;
    this.isPlaying = true;
    this.server.emit("postPlay", this.id, this.playStreamPath, this.playArgs);
  ...
  onDeleteStream(invokeMessage) {
    if (invokeMessage.streamId == this.playStreamId) {
      if (this.isIdling) {
        this.server.idlePlayers.delete(this.id);
        this.isIdling = false;
      } else {
        let publisherId = this.server.publishers.get(this.playStreamPath);
        if (publisherId != null) {
          this.server.sessions.get(publisherId).players.delete(this.id);
        }
        this.server.emit("donePlay", this.id, this.playStreamPath, this.playArgs);
        this.isPlaying = false;
      }
    ...
    if (invokeMessage.streamId == this.publishStreamId) {
      if (this.isPublishing) {
        ...
        this.server.emit("donePublish", this.id, this.publishStreamPath, this.publishArgs, this.publishStreamName, this.publishMetaData);
        if (this.isStarting) {
        ...
        for (let playerId of this.players) {
          let playerSession = this.server.sessions.get(playerId);
          this.server.idlePlayers.add(playerId);
        ...
        this.server.publishers.delete(this.publishStreamPath);
