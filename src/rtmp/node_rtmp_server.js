...
class NodeRtmpServer extends EventEmitter {
  constructor() {
    super();
    this.sessions = new Map();
    this.publishers = new Map();
    this.idlePlayers = new Set();
    // @todo : config.rtmp.get('authentification') && config.rtmp.get('authentification.host')
    config.rtmp.port = this.port = config.rtmp.port ? config.rtmp.port : RTMP_PORT;
    this.tcpServer = Net.createServer((socket) => {
      let session = new NodeRtmpSession( socket, this);
      session.run();
    });
    ...
    if (config.rtmp.ssl){
      ...
      try {
        ...
        this.tlsServer = Tls.createServer(options, (socket) => {
          let session = new NodeRtmpSession(config, socket, this);
          session.run();
        });
      } catch (e) {
